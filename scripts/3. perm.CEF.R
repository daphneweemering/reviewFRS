...
X <- pbmclapply(1:s, function(i){
  ...
  # - ################################ DATA ################################ - #
  D <- D.ADJ.CEF(TE = TE, D = "CEF", TRT.cenrate = TRT.cenrate)

  D1 <- list(WD = D$WD, LOCF = D$LOCF, ZERO = D$ZERO, COMP = D$COMP,
             WD.AGG = D$WD.AGG, WD.JR = D$WD.JR, WD.KAJI = D$WD.KAJI, 
             WD.KAUF = D$WD.KAUF, WD.MILL = D$WD.MILL, LOCF.MORA = D$LOCF.MORA)

  D2 <- list(LD = D$LD, LOCF2 = D$LOCF2)
  ...
  est <- function(x){
    if (class(x)[1] == "lm"){
      ...
      if ("TRT:VISIT6" %in% names(coefficients(x))){
        e <- unname(coefficients(x)["TRT:VISIT6"])
      } else if ("VISIT6:TRT" %in% names(coefficients(x))){
        e <- unname(coefficients(x)["VISIT6:TRT"])
      }
  ...
  e8 <- as.data.frame(contrast(emmeans(lmer(CFB ~ TRT * VISIT + (1 | ID), data = LD[!is.na(LD$CFB), ]), ~ TRT), method = "pairwise"))$estimate
  e8 <- e8 * - 1
  ...
  m9 <- lmer(TOT ~ TIME + TRT + TRT:TIME + (TIME|ID), data = D2$LD, control = lmerControl(optimizer = "nlminbwrap"))
  e9 <- est(m9)
  ...
  m11 <- lmer(TOT ~ TIME + TRT + TRT:TIME + (TIME|ID), data = D2$LD, control = lmerControl(optimizer = "nlminbwrap"))
  e11 <- est(m11)
  ...
  e13 <- as.data.frame(contrast(emmeans(lmer(CFB ~ TRT * VISIT + (1 | ID), data = LOCF2[!is.na(LOCF2$CFB), ]), ~ TRT), method = "pairwise"))$estimate
  e13 <- e13 * -1
  ...
  m14 <- lm(V6.TOT ~ TRT, data = D1$COMP)
  e14 <- est(m14)
  ...
  m17 <- lmer(TOT ~ TIME + TRT + TIME:TRT + (TIME|ID), data = D2$LD, control = lmerControl(optimizer = "nlminbwrap"))
  e17 <- est(m17)
  p17 <- drop1(m17, ~ TIME:TRT, test = "Chisq")[2, 4]

  # 18. -- Juntas-Morales - 2020 - MD1003 - PMID: 32140672 ---------------------
  m18 <- coin::wilcox_test(V6.CFB ~ as.factor(TRT), data = D1$ZERO, distribution = "exact")
  e18 <- m18@statistic@teststatistic
  p18 <- coin::pvalue(m18)

  # 19. -- Kaji - 2019 - methylcobalamin - PMID: 30636701 ----------------------
  m19 <- coin::wilcox_test(V6.CFB ~ as.factor(TRT), data = D1$WD.KAJI, distribution = "exact")
  e19 <- m19@statistic@teststatistic
  p19 <- coin::pvalue(m19)

  # 20. -- Kaufmann - 2009 - COQ10 - PMID: 19743457 ----------------------------
  m20 <- lm(V6.CFB ~ TRT, data = D1$WD.KAUF)
  e20 <- est(m20)
  ...
  e22 <- m22@statistic@teststatistic
  p22 <- coin::pvalue(m22)
  ...
  m26 <- lmer(TOT ~ TIME + TIME:TRT + AGE + I(ifelse(RACE != "WHITE" & RACE != "BLACK", "OTHER", RACE)) +
                SEX + DISDUR + FAMILIAL + RILUSE + ONSET + I(ESCORIAL == "DEF") +
                BSLN + FVCL + (TIME | ID), data = D2$LD,
  ...
  m27.r <-lapply(1:5, function(i){
    lm(RANK ~ DISDUR + V0.TOT + RILUSE, data = D1$WD.MILL[D1$WD.MILL$.imp == i, ])
  })

  LL.full <- sapply(m27, logLik)
  LL.red <- sapply(m27.r, logLik)

  LRT <- -2 * (LL.red - LL.full)

  library(miceadds)
  p27.1 <- miceadds::micombine.chisquare(LRT, 1)[[2]]
  ########

  estimates <- pool(m27)
  e27 <- summary(estimates)[2, 2]
  p27 <- summary(estimates)[2, 6]

  # 28. -- Miller - 2022 - NP001 - PMID: 35098554 ------------------------------
  m28 <- coin::wilcox_test(V6.CFB ~ as.factor(TRT), data = D1$COMP, distribution = "exact")
  e28 <- m28@statistic@teststatistic
  p28 <- coin::pvalue(m28)
  ...
  m38 <- lm(V6.CFB ~ TRT, data = D1$LOCF)
  e38 <- est(m38)
  p38 <- drop1(m38, test = "Chisq")[2, 5]

  # 39. -- Shefner - 2016 - tirasemtiv - PMID: 26982815 ------------------------
  m39 <- mmrm(CFB ~ TRT + VISIT + TRT:VISIT + BSLN + BSLN:VISIT + RILUSE +
                us(VISIT|ID), data = D2$LD[!D2$LD$VISIT == 0, ])
  e39 <- est(m39)
  p39 <- unname(unlist(summary(emmeans(m39, specs = pairwise ~ TRT, at = list(VISIT = c("6")))$contrast)[6]))

  # 40. -- Shibuya - 2015 - mexiletine - PMID: 25960085 ------------------------
  m40 <- lm(V6.CFB ~ TRT + I(V0.TOT >= 37) + I(AGE <= 65), data = D1$COMP)
  e40 <- est(m40)
  ...
  m42 <- mmrm(CFB ~ TRT:VISIT + VISIT + BSLN + us(VISIT|ID), data = D2$LOCF2[!D2$LOCF2$VISIT == 0, ])
  e42 <- est(m42)
  p42 <- unname(unlist(summary(emmeans(m42, specs = pairwise ~ TRT, at = list(VISIT = c("6")))$contrast)[6]))
  ...
  p43 <- pchisq(p43, df = 7, lower.tail = F)
